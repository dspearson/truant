...
impl TrampolineGenerator for AArch64TrampolineGenerator {
    ...
    fn generate_trampoline(
        ...
        // ── Save registers ──────────────────────────────────────────────
        // STP x9, x10, [sp, #-16]!
        code.extend_from_slice(&asm::encode_stp_push(9, 10));
        // STP x11, x12, [sp, #-16]!
        code.extend_from_slice(&asm::encode_stp_push(11, 12));
        ...
        asm::emit_adr_auto(&mut code, 11, adr1_pc, data_va);

        // LDR x10, [x11]  (x10 = shm_ptr)
        code.extend_from_slice(&asm::encode_ldr_reg(10, 11));
        ...
        code.extend_from_slice(&asm::encode_ldrh_imm(12, 11, PREV_LOC_OFFSET as u32));

        // ── Compute edge = (block_id XOR prev_loc) & 0xFFFF ──────────────
        // MOVZ x11, #block_id  (block_id is u16, fits in 16-bit immediate)
        code.extend_from_slice(&asm::encode_movz(11, block_id as u16));

        // EOR x11, x11, x12  (edge = block_id XOR prev_loc)
        code.extend_from_slice(&asm::encode_eor_reg(11, 11, 12));

        // AND x11, x11, #0xFFFF (mask to 16 bits)
        code.extend_from_slice(&asm::encode_and_0xffff(11, 11));

        // ── Increment shm_map[edge] ─────────────────────────────────────
        // x10 = shm_ptr (base of coverage map), x11 = edge index
        // ADD x10, x10, x11  ; x10 = &shm_map[edge]
        code.extend_from_slice(&asm::encode_add_reg(10, 10, 11));

        // LDRB w11, [x10]   ; w11 = current count
        code.extend_from_slice(&asm::encode_ldrb_reg(11, 10));

        // ADD w11, w11, #1  (no flags — ADD without S suffix)
        code.extend_from_slice(&asm::encode_add_w_imm1(11, 11));

        // STRB w11, [x10]   ; store back
        code.extend_from_slice(&asm::encode_strb_reg(11, 10));

        // ── Update prev_loc = block_id >> 1 ─────────────────────────────
        // Need data_ptr again — recompute address.
        let adr2_pc = trampoline_va + code.len() as u64;
        asm::emit_adr_auto(&mut code, 11, adr2_pc, data_va);

        // MOVZ x10, #(block_id >> 1)
        code.extend_from_slice(&asm::encode_movz(10, (block_id >> 1) as u16));

        // STRH w10, [x11, #PREV_LOC_OFFSET]
        code.extend_from_slice(&asm::encode_strh_imm(10, 11, PREV_LOC_OFFSET as u32));
        ...
        // ── Restore registers ─────────────────────────────────────────────
        // LDP x11, x12, [sp], #16
        code.extend_from_slice(&asm::encode_ldp_pop(11, 12));
        // LDP x9, x10, [sp], #16
        code.extend_from_slice(&asm::encode_ldp_pop(9, 10));
        ...
        code.extend_from_slice(&asm::encode_b(imm26));
    ...
    fn generate_init_code(
        ...
        code.extend_from_slice(&asm::encode_stp_push(0, 31 /* xzr */));

        // SUB sp, sp, #4192
        // 4192 = 0x1060, but SUB imm12 only supports 0..4095.
        // Split: SUB sp, sp, #4096; SUB sp, sp, #96
        code.extend_from_slice(&asm::encode_sub_sp_imm(4096));
        code.extend_from_slice(&asm::encode_sub_sp_imm(96));

        // STP x29, x30, [sp, #0]  (frame pointer + link register)
        code.extend_from_slice(&asm::encode_stp_offset(29, 30, 0));
        // STP x27, x28, [sp, #16]
        code.extend_from_slice(&asm::encode_stp_offset(27, 28, 16));
        // STP x25, x26, [sp, #32]
        code.extend_from_slice(&asm::encode_stp_offset(25, 26, 32));
        // STP x23, x24, [sp, #48]
        code.extend_from_slice(&asm::encode_stp_offset(23, 24, 48));
        // STP x21, x22, [sp, #64]
        code.extend_from_slice(&asm::encode_stp_offset(21, 22, 64));
        // STP x19, x20, [sp, #80]
        code.extend_from_slice(&asm::encode_stp_offset(19, 20, 80));
        ...
        code.extend_from_slice(&asm::encode_nop()); // placeholder: ADR x1, path_string

        // x0 = AT_FDCWD (-100)
        asm::emit_mov32(&mut code, 0, AT_FDCWD_U32);
        // x1 already set above (will be patched)
        // x2 = O_RDONLY = 0
        code.extend_from_slice(&asm::encode_movz(2, 0));
        // x3 = 0 (mode, unused for O_RDONLY)
        code.extend_from_slice(&asm::encode_movz(3, 0));
        // syscall openat
        asm::emit_syscall(&mut code, SYS_OPENAT);
        // x0 = fd (or error if negative)

        // Save fd in x19 (callee-saved, won't be clobbered by further syscalls).
        code.extend_from_slice(&asm::encode_mov_reg(19, 0));

        // ── Read from fd into stack buffer [sp + FRAME_REGS] ────────────
        // sys_read(fd, buf, 4096) → bytes_read in x0
        // x0 = fd (from x19)
        code.extend_from_slice(&asm::encode_mov_reg(0, 19));
        // x1 = buf = sp + FRAME_REGS
        code.extend_from_slice(&asm::encode_add_imm(1, 31 /*sp*/, FRAME_REGS));
        // x2 = 4096
        asm::emit_mov32(&mut code, 2, BUF_SIZE);
        asm::emit_syscall(&mut code, SYS_READ);
        // x0 = bytes_read; save in x20
        code.extend_from_slice(&asm::encode_mov_reg(20, 0));

        // ── Close fd ─────────────────────────────────────────────────────
        code.extend_from_slice(&asm::encode_mov_reg(0, 19));
        asm::emit_syscall(&mut code, SYS_CLOSE);
        ...
        code.extend_from_slice(&asm::encode_add_imm(21, 31 /*sp*/, FRAME_REGS));
        // x22 = x21 (scan pointer)
        code.extend_from_slice(&asm::encode_mov_reg(22, 21));
        // x23 = x21 + x20 (end)
        code.extend_from_slice(&asm::encode_add_reg(23, 21, 20));
        // x24 = 0 (parsed shmid, default 0)
        code.extend_from_slice(&asm::encode_movz(24, 0));
        ...
        code.extend_from_slice(&asm::encode_nop()); // placeholder: LDR x25, #needle_a
        let needle_b_pos = code.len();
        code.extend_from_slice(&asm::encode_nop()); // placeholder: LDR x26, #needle_b
        ...
        code.extend_from_slice(&asm::encode_cmp_reg(22, 23));
        ...
        code.extend_from_slice(&asm::encode_ldrb_reg(27, 22));

        // Check if first byte is '_' (0x5F)
        // CMP w27, #0x5F
        code.extend_from_slice(&asm::encode_cmp_w_imm(27, 0x5F));
        // B.NE .next_byte
        ...
        code.extend_from_slice(&asm::encode_ldr_reg(27, 22));
        // CMP x27, x25
        code.extend_from_slice(&asm::encode_cmp_reg(27, 25));
        // B.NE .next_byte
        ...
        code.extend_from_slice(&asm::encode_ldr_imm(27, 22, 8));
        let tmp = code.len() - 4;
        // Replace the last instruction with a 32-bit LDR
        let ldr_w = asm::encode_ldr_w_imm(27, 22, 8);
        code[tmp..tmp + 4].copy_from_slice(&ldr_w);

        // CMP w27, w26 (lower 32 bits = "M_ID")
        code.extend_from_slice(&asm::encode_cmp_w_reg(27, 26));
        // B.NE .next_byte
        ...
        code.extend_from_slice(&asm::encode_ldrb_reg(27, 22));
        let tmp2 = code.len() - 4;
        let ldrb_12 = asm::encode_ldrb_imm(27, 22, 12);
        code[tmp2..tmp2 + 4].copy_from_slice(&ldrb_12);

        code.extend_from_slice(&asm::encode_cmp_w_imm(27, b'='.into()));
        ...
        code.extend_from_slice(&asm::encode_add_imm(22, 22, 13));
        ...
        code.extend_from_slice(&asm::encode_movz(25, 10));

        // .parse_loop:
        let parse_loop = code.len();

        // LDRB w27, [x22], #1  (post-index load)
        code.extend_from_slice(&asm::encode_ldrb_post_inc(27, 22));

        // SUB w27, w27, #'0'
        code.extend_from_slice(&asm::encode_sub_w_imm(27, 27, b'0'.into()));

        // CMP w27, #10  (check if digit 0-9)
        code.extend_from_slice(&asm::encode_cmp_w_imm(27, 10));
        ...
        code.extend_from_slice(&asm::encode_mul(24, 24, 25));
        // ADD x24, x24, x27 (zero-extend w27)
        code.extend_from_slice(&asm::encode_add_uxtw(24, 24, 27));
        ...
        code.extend_from_slice(&asm::encode_add_imm(22, 22, 1));
        ...
        code.extend_from_slice(&asm::encode_b(scan_loop_back));
        ...
        code.extend_from_slice(&asm::encode_b(0)); // placeholder: B .epilogue
        ...
        let encoded_b = asm::encode_b(b_after_delta / 4);
        ...
        code.extend_from_slice(&asm::encode_mov_reg(0, 24)); // x0 = shmid
        code.extend_from_slice(&asm::encode_movz(1, 0)); // x1 = 0 (NULL)
        code.extend_from_slice(&asm::encode_movz(2, 0)); // x2 = 0
        asm::emit_syscall(&mut code, SYS_SHMAT);
        ...
        asm::emit_adr_auto(&mut code, 20, adr_datava_pc, data_va);

        // STR x19, [x20]  (shm_ptr at data_va+0)
        code.extend_from_slice(&asm::encode_str_reg(19, 20));

        // Zero-initialize prev_loc at [data_va + PREV_LOC_OFFSET]
        code.extend_from_slice(&asm::encode_strh_zero(20, PREV_LOC_OFFSET as u32));
        ...
        let encoded_b_epi = asm::encode_b(b_epi_delta as i32);
        ...
        // LDP x19, x20, [sp, #80]
        code.extend_from_slice(&asm::encode_ldp_offset(19, 20, 80));
        // LDP x21, x22, [sp, #64]
        code.extend_from_slice(&asm::encode_ldp_offset(21, 22, 64));
        // LDP x23, x24, [sp, #48]
        code.extend_from_slice(&asm::encode_ldp_offset(23, 24, 48));
        // LDP x25, x26, [sp, #32]
        code.extend_from_slice(&asm::encode_ldp_offset(25, 26, 32));
        // LDP x27, x28, [sp, #16]
        code.extend_from_slice(&asm::encode_ldp_offset(27, 28, 16));
        // LDP x29, x30, [sp, #0]
        code.extend_from_slice(&asm::encode_ldp_offset(29, 30, 0));

        // ADD sp, sp, #FRAME_SIZE
        code.extend_from_slice(&asm::encode_add_sp_imm(96));
        code.extend_from_slice(&asm::encode_add_sp_imm(4096));

        // LDP x0, xzr, [sp], #16  — restore x0 (post-index, sp += 16)
        code.extend_from_slice(&asm::encode_ldp_pop(0, 31 /* xzr */));
        ...
        if delta.abs() < (128 << 20) {
            ...
            code.extend_from_slice(&asm::encode_add_imm(16, 16, page_off));
            code.extend_from_slice(&asm::encode_br(16));
        }

        // ── Literal pool ─────────────────────────────────────────────────
        // Align to 8 bytes.
        while !code.len().is_multiple_of(8) {
            code.extend_from_slice(&asm::encode_nop());
        }
        ...
        asm::patch_adr(&mut code, adr_path_pos, path_lit_offset, 1);
        ...
        asm::patch_ldr_literal(&mut code, needle_a_pos, init_va, needle_a_lit_offset, 25);

        // Patch LDR x26, #needle_b (needle_b_pos → needle_b_lit_offset)
        asm::patch_ldr_literal(&mut code, needle_b_pos, init_va, needle_b_lit_offset, 26);
    ...
    fn generate_so_init_code(
        ...
        code.extend_from_slice(&asm::encode_movz(3, 0));
        asm::emit_syscall(&mut code, SYS_OPENAT);
        code.extend_from_slice(&asm::encode_mov_reg(19, 0)); // fd in x19

        // ── Read environ ──────────────────────────────────────────────────
        code.extend_from_slice(&asm::encode_mov_reg(0, 19));
        code.extend_from_slice(&asm::encode_add_imm(1, 31, FRAME_REGS));
        asm::emit_mov32(&mut code, 2, BUF_SIZE);
        asm::emit_syscall(&mut code, SYS_READ);
        code.extend_from_slice(&asm::encode_mov_reg(20, 0)); // bytes_read in x20
        ...
        code.extend_from_slice(&asm::encode_add_imm(21, 31, FRAME_REGS));
        code.extend_from_slice(&asm::encode_mov_reg(22, 21));
        code.extend_from_slice(&asm::encode_add_reg(23, 21, 20));
        code.extend_from_slice(&asm::encode_movz(24, 0));
        ...
        code.extend_from_slice(&asm::encode_cmp_w_imm(27, b'=' as u32));
        let bne4 = code.len();
        code.extend_from_slice(&[0x61, 0x00, 0x00, 0x54]);
        ...
        code.extend_from_slice(&asm::encode_ldrb_post_inc(27, 22));
        code.extend_from_slice(&asm::encode_sub_w_imm(27, 27, b'0' as u32));
        code.extend_from_slice(&asm::encode_cmp_w_imm(27, 10));
        ...
        code.extend_from_slice(&asm::encode_b((bpl_delta / 4) as i32));
        ...
        code.extend_from_slice(&asm::encode_b((slb_delta / 4) as i32));
        ...
        let enc = asm::encode_b(b_skip_delta / 4);
        code[b_skip_notfound..b_skip_notfound + 4].copy_from_slice(&enc);

        // ── SHM attach ────────────────────────────────────────────────────
        code.extend_from_slice(&asm::encode_mov_reg(0, 24));
        code.extend_from_slice(&asm::encode_movz(1, 0));
        code.extend_from_slice(&asm::encode_movz(2, 0));
        asm::emit_syscall(&mut code, SYS_SHMAT);
        ...
        code.extend_from_slice(&asm::encode_mov_reg(19, 0)); // shm_ptr in x19

        // ── Store shm_ptr at data_va ──────────────────────────────────────
        // Compute data_va address (PC-relative, PIE-safe).
        let adr_datava_pc2 = init_va + code.len() as u64;
        asm::emit_adr_auto(&mut code, 20, adr_datava_pc2, data_va);

        code.extend_from_slice(&asm::encode_str_reg(19, 20)); // STR x19, [x20] -- shm_ptr
        code.extend_from_slice(&asm::encode_strh_zero(20, PREV_LOC_OFFSET as u32)); // clear prev_loc

        // ── Chain to original DT_INIT if present ─────────────────────────
        if let Some(dt_init_va) = dt_init {
            let adr_dtinit_pc = init_va + code.len() as u64;
            asm::emit_adr_auto(&mut code, 16, adr_dtinit_pc, dt_init_va);
            code.extend_from_slice(&asm::encode_blr(16)); // BLR x16
        }
        ...
        let enc_epi = asm::encode_b(b_soepi_delta as i32);
        ...
        code.extend_from_slice(&asm::encode_ret());
...
fn emit_forkserver_arm64(code: &mut Vec<u8>) {
    ...
    code.extend_from_slice(&asm::encode_sub_sp_imm(16));

    // Write 4-byte hello to FORKSRV_FD+1 (fd=199)
    // Store 0 at [sp] for the hello value.
    code.extend_from_slice(&asm::encode_movz(27, 0)); // x27 = 0
    code.extend_from_slice(&asm::encode_str_sp_imm(27, 0)); // STR x27, [sp]

    // sys_write(199, &hello, 4)
    asm::emit_mov32(code, 0, FORKSRV_FD + 1); // fd
    code.extend_from_slice(&asm::encode_add_imm(1, 31, 0)); // buf = sp + 0
    code.extend_from_slice(&asm::encode_movz(2, 4)); // len = 4
    asm::emit_syscall(code, SYS_WRITE);
    ...
    asm::emit_mov32(code, 0, FORKSRV_FD);
    code.extend_from_slice(&asm::encode_add_imm(1, 31, 0));
    code.extend_from_slice(&asm::encode_movz(2, 4));
    asm::emit_syscall(code, SYS_READ);

    // If x0 <= 0: exit (pipe closed)
    code.extend_from_slice(&asm::encode_cmp_imm0(0)); // CMP x0, #0
    ...
    code.extend_from_slice(&asm::encode_movz(0, SIGCHLD as u16)); // x0 = SIGCHLD
    code.extend_from_slice(&asm::encode_movz(1, 0)); // x1 = 0 (child_stack = NULL → share parent stack)
    code.extend_from_slice(&asm::encode_movz(2, 0)); // x2 = 0 (parent_tid)
    code.extend_from_slice(&asm::encode_movz(3, 0)); // x3 = 0 (tls)
    code.extend_from_slice(&asm::encode_movz(4, 0)); // x4 = 0 (child_tid)
    asm::emit_syscall(code, SYS_CLONE);
    code.extend_from_slice(&asm::encode_mov_reg(27, 0)); // x27 = child_pid or 0

    // CBZ x27, .child_path (x27 == 0 means we're in child)
    let cbz_child_pos = code.len();
    code.extend_from_slice(&asm::encode_cbz(27, 0)); // placeholder

    // ── Parent path ───────────────────────────────────────────────────────
    // Write child_pid to FORKSRV_FD+1
    // Store x27 (pid) at [sp]
    code.extend_from_slice(&asm::encode_str_sp_imm(27, 0));
    // sys_write(199, &child_pid, 4)
    asm::emit_mov32(code, 0, FORKSRV_FD + 1);
    code.extend_from_slice(&asm::encode_add_imm(1, 31, 0));
    code.extend_from_slice(&asm::encode_movz(2, 4));
    asm::emit_syscall(code, SYS_WRITE);

    // sys_wait4(child_pid, &status, 0, NULL)
    code.extend_from_slice(&asm::encode_mov_reg(0, 27)); // x0 = child_pid
    code.extend_from_slice(&asm::encode_add_imm(1, 31, 8)); // x1 = &status = sp+8
    code.extend_from_slice(&asm::encode_movz(2, 0)); // x2 = 0 (WNOHANG=0)
    code.extend_from_slice(&asm::encode_movz(3, 0)); // x3 = NULL
    asm::emit_syscall(code, SYS_WAIT4);

    // Write exit status to FORKSRV_FD+1
    asm::emit_mov32(code, 0, FORKSRV_FD + 1);
    code.extend_from_slice(&asm::encode_add_imm(1, 31, 8));
    code.extend_from_slice(&asm::encode_movz(2, 4));
    asm::emit_syscall(code, SYS_WRITE);
    ...
    code.extend_from_slice(&asm::encode_movz(0, 0));
    asm::emit_syscall(code, SYS_EXIT);
    ...
    let cbz_enc = asm::encode_cbz(27, cbz_delta);
    ...
    code.extend_from_slice(&asm::encode_add_sp_imm(16));
...
pub fn generate_persistent_wrapper_aarch64(
    ...
    let emit_adr_placeholder = |code: &mut Vec<u8>, reg: u32| -> usize {
        let pos = code.len();
        code.extend_from_slice(&asm::encode_adr(reg, 0));
        pos
    };
    ...
    if include_forkserver {
        ...
        code.extend_from_slice(&asm::encode_ldrb_imm(10, 9, OFF_FS_STARTED));
        // CBNZ w10, .persistent_section
        let cbnz_persistent_pos = code.len();
        code.extend_from_slice(&asm::encode_cbnz(10, 0)); // placeholder

        // Set forkserver_started = 1
        code.extend_from_slice(&asm::encode_movz(10, 1));
        code.extend_from_slice(&asm::encode_strb_imm(10, 9, OFF_FS_STARTED));
        ...
        for reg in 0..8u32 {
            code.extend_from_slice(&asm::encode_str_imm_base(reg, 9, OFF_SAVE_ARGS + reg * 8));
        }

        // Save callee-saved regs to stack
        code.extend_from_slice(&asm::encode_sub_sp_imm(160));
        for i in 0..5i32 {
            code.extend_from_slice(&asm::encode_stp_offset(
                (19 + i * 2) as u32,
                (20 + i * 2) as u32,
                i * 16,
            ));
        }
        code.extend_from_slice(&asm::encode_stp_offset(29, 30, 80)); // fp, lr

        // sub sp, sp, 16 (scratch space)
        code.extend_from_slice(&asm::encode_sub_sp_imm(16));

        // Hello: write(FORKSRV_FD+1, &zero, 4)
        code.extend_from_slice(&asm::encode_movz(10, 0));
        code.extend_from_slice(&asm::encode_str_sp_imm(10, 0)); // [sp] = 0
        asm::emit_mov32(&mut code, 0, FORKSRV_FD + 1);
        code.extend_from_slice(&asm::encode_add_imm(1, 31, 0)); // x1 = sp
        code.extend_from_slice(&asm::encode_movz(2, 4));
        asm::emit_syscall(&mut code, SYS_WRITE);
        ...
        asm::emit_mov32(&mut code, 0, FORKSRV_FD);
        code.extend_from_slice(&asm::encode_add_imm(1, 31, 0));
        code.extend_from_slice(&asm::encode_movz(2, 4));
        asm::emit_syscall(&mut code, SYS_READ);
        ...
        code.extend_from_slice(&asm::encode_ldrb_imm(10, 9, OFF_CHILD_STOPPED));
        // CBZ w10, .do_fork
        let cbz_do_fork_pos = code.len();
        code.extend_from_slice(&asm::encode_cbz(10, 0)); // placeholder

        // Resume stopped child: kill(x27, SIGCONT=18)
        code.extend_from_slice(&asm::encode_mov_reg(0, 27)); // pid
        code.extend_from_slice(&asm::encode_movz(1, 18)); // SIGCONT
        asm::emit_syscall(&mut code, SYS_KILL);
        ...
        code.extend_from_slice(&asm::encode_b(0)); // placeholder

        // .do_fork:
        let do_fork = code.len();
        let cd = (do_fork as i64 - cbz_do_fork_pos as i64) as i32;
        code[cbz_do_fork_pos..cbz_do_fork_pos + 4].copy_from_slice(&asm::encode_cbz(10, cd));

        // clone(SIGCHLD, 0, 0, 0, 0) — Linux ARM64 has no fork()
        asm::emit_mov32(&mut code, 0, SIGCHLD);
        code.extend_from_slice(&asm::encode_movz(1, 0)); // newsp=0
        code.extend_from_slice(&asm::encode_movz(2, 0)); // parent_tidptr=0
        code.extend_from_slice(&asm::encode_movz(3, 0)); // tls=0
        code.extend_from_slice(&asm::encode_movz(4, 0)); // child_tidptr=0
        asm::emit_syscall(&mut code, SYS_CLONE);
        // x0 = pid in parent, 0 in child
        code.extend_from_slice(&asm::encode_mov_reg(27, 0)); // save pid

        // CBZ x0, .child (x0==0 means child)
        let cbz_child_pos = code.len();
        code.extend_from_slice(&asm::encode_cbz(0, 0)); // placeholder
        ...
        code[b_waitpid_pos..b_waitpid_pos + 4].copy_from_slice(&asm::encode_b(brel));

        // wait4(x27, &status, WUNTRACED=2, NULL)
        code.extend_from_slice(&asm::encode_mov_reg(0, 27));
        code.extend_from_slice(&asm::encode_add_imm(1, 31, 4)); // &status at [sp+4]
        code.extend_from_slice(&asm::encode_movz(2, 2)); // WUNTRACED
        code.extend_from_slice(&asm::encode_movz(3, 0));
        asm::emit_syscall(&mut code, SYS_WAIT4);

        // Check WIFSTOPPED: (status & 0xFF) == 0x7F
        code.extend_from_slice(&asm::encode_ldr_w_imm(10, 31, 4)); // w10 = [sp+4]
        code.extend_from_slice(&[0x4A, 0x1D, 0x00, 0x12]); // AND w10, w10, #0xFF
        code.extend_from_slice(&asm::encode_cmp_w_imm(10, 0x7F));
        // B.NE .not_stopped
        ...
        code.extend_from_slice(&asm::encode_movz(10, 1));
        code.extend_from_slice(&asm::encode_strb_imm(10, 9, OFF_CHILD_STOPPED));
        code.extend_from_slice(&asm::encode_movz(10, 0));
        code.extend_from_slice(&asm::encode_str_sp_imm(10, 4)); // status=0
        // B .report_status
        ...
        code[b_report_pos..b_report_pos + 4].copy_from_slice(&asm::encode_b(brel));

        // write(FORKSRV_FD+1, &status, 4)
        asm::emit_mov32(&mut code, 0, FORKSRV_FD + 1);
        code.extend_from_slice(&asm::encode_add_imm(1, 31, 4));
        code.extend_from_slice(&asm::encode_movz(2, 4));
        asm::emit_syscall(&mut code, SYS_WRITE);

        // B .fork_loop
        let bl2 = code.len();
        code.extend_from_slice(&asm::encode_b(((fork_loop as i64 - bl2 as i64) / 4) as i32));

        // .child:
        let child = code.len();
        let cd = (child as i64 - cbz_child_pos as i64) as i32;
        code[cbz_child_pos..cbz_child_pos + 4].copy_from_slice(&asm::encode_cbz(0, cd));
        ...
        for i in 0..5i32 {
            code.extend_from_slice(&asm::encode_ldp_offset(
                (19 + i * 2) as u32,
                (20 + i * 2) as u32,
                i * 16,
            ));
        }
        code.extend_from_slice(&asm::encode_ldp_offset(29, 30, 80));
        code.extend_from_slice(&asm::encode_add_sp_imm(160));
        ...
        for reg in 0..8u32 {
            code.extend_from_slice(&asm::encode_ldr_imm(reg, 9, OFF_SAVE_ARGS + reg * 8));
        }

        // B .persistent_section (placeholder)
        let b_persistent_pos = code.len();
        code.extend_from_slice(&asm::encode_b(0));

        // .exit_parent: exit(0)
        code.extend_from_slice(&asm::encode_movz(0, 0));
        asm::emit_syscall(&mut code, SYS_EXIT);
    ...
    code.extend_from_slice(&asm::encode_ldrb_imm(10, 9, OFF_FIRST_PASS));
    ...
    code.extend_from_slice(&asm::encode_movz(10, 0));
    code.extend_from_slice(&asm::encode_strb_imm(10, 9, OFF_FIRST_PASS));

    // Save SP, LR, x0-x7 to persistent data
    code.extend_from_slice(&asm::encode_mov_reg(10, 31)); // x10 = sp
    code.extend_from_slice(&asm::encode_str_imm_base(10, 9, OFF_SAVE_SP));
    code.extend_from_slice(&asm::encode_str_imm_base(30, 9, OFF_SAVE_LR)); // lr
    ...
    // Set counter
    asm::emit_mov32(&mut code, 10, persistent_count);
    code.extend_from_slice(&asm::encode_str_w_imm_base(10, 9, OFF_COUNTER));

    // === Section B: .setup_and_run ===
    let setup_and_run = code.len();
    let cd = (setup_and_run as i64 - cbz_setup_pos as i64) as i32;
    code[cbz_setup_pos..cbz_setup_pos + 4].copy_from_slice(&asm::encode_cbz(10, cd));

    // Set LR to iteration_boundary so function return comes back here
    let adr_lr_iter_pos = code.len();
    code.extend_from_slice(&asm::encode_adr(30, 0)); // placeholder
    ...
    code.extend_from_slice(&asm::encode_b(brel));
    ...
    code[adr_lr_iter_pos..adr_lr_iter_pos + 4].copy_from_slice(&asm::encode_adr(30, adr_off));
    ...
    code.extend_from_slice(&asm::encode_ldr_w_imm(10, 9, OFF_COUNTER));
    // SUBS w10, w10, #1
    code.extend_from_slice(&asm::encode_subs_w_imm(10, 10, 1));
    code.extend_from_slice(&asm::encode_str_w_imm_base(10, 9, OFF_COUNTER));
    ...
    // getpid()
    asm::emit_syscall(&mut code, SYS_GETPID);
    // kill(self, SIGSTOP=19)
    code.extend_from_slice(&asm::encode_movz(1, SIGSTOP as u16));
    asm::emit_syscall(&mut code, SYS_KILL);
    ...
    code.extend_from_slice(&asm::encode_strh_zero(9, 0)); // STRH wzr, [x9]
    ...
    code.extend_from_slice(&asm::encode_ldr_imm(10, 9, OFF_SAVE_SP));
    code.extend_from_slice(&asm::encode_mov_reg(31, 10)); // sp = x10
    code.extend_from_slice(&asm::encode_ldr_imm(30, 9, OFF_SAVE_LR)); // lr
    ...
    for entry in &lit_pool {
        ...
        code[entry.instr_pos..entry.instr_pos + 4].copy_from_slice(&asm::encode_adr(rd, offset));
    }
...
mod tests {
    ...
    fn test_encode_sub_sp_imm_small() {
        ...
        let bytes = asm::encode_sub_sp_imm(96);
    ...
    fn test_encode_sub_sp_imm_4096() {
        ...
        let bytes = asm::encode_sub_sp_imm(4096);
        let word = u32::from_le_bytes(bytes);
        let sh = (word >> 22) & 1;
        let imm12 = (word >> 10) & 0xFFF;
    ...
    fn test_encode_add_sp_imm_4096() {
        ...
        let bytes = asm::encode_add_sp_imm(4096);
        let word = u32::from_le_bytes(bytes);
        let sh = (word >> 22) & 1;
        let imm12 = (word >> 10) & 0xFFF;
        assert_eq!(sh, 1, "sh should be 1 for 4096");
