...
pub fn generate_macho_persistent_wrapper_aarch64(
    ...
    // Emit displaced instructions (raw copy for ARM64 — 4-byte aligned instructions)
    // ARM64 instructions are position-independent (B/BL have relative offsets that need fixing,
    // but for function entry instructions they're typically non-branching).
    // For safety, we copy the raw bytes and hope they're position-independent.
    // TODO: proper ARM64 instruction relocation for PC-relative instructions
    code.extend_from_slice(displaced_bytes);
